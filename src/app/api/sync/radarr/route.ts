...
export async function POST(req: NextRequest) {
  ...
  return withCronRunRecording("radarr-sync", async () => {
    ...
    let available = 0;
    try {
      // Fetch from every configured Radarr instance (the default, the legacy 4K one
      // and any named ones), at most FETCH_CONCURRENCY at a time (guardrail 31).
      // getRadarrWantedTmdbIds returns empty sets for an unconfigured instance and
      // null when a real fetch failed.
      ...
      // leaves THAT instance's rows intact (guardrail 13) so one failure never empties another's cache.
      const writable = fetched.flatMap((f) => (f.result ? [{ slug: f.slug, result: f.result }] : []));
      // Advisory lock 1001,1 makes the Radarr webhook handler and the sync orchestrator
      // wait for this rewrite, so they never see a half-written cache.
