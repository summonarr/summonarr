...
import { qualityMultiplier, withTmdbTerm } from "@/lib/recommendation-quality";
import {
  readGraphSuggestions,
  readTitleQuality,
  refreshRecommendationGraph,
  type GraphRefreshResult,
} from "@/lib/recommendation-graph";

// qualityScoreOf moved to recommendation-quality.ts (the graph precomputes a
// verdict per TITLE, so the scorer can no longer live behind `server-only`).
// Re-exported here because it is this engine's prior conceptually, and every
// existing caller and test imports it from this module.
export { qualityScoreOf } from "@/lib/recommendation-quality";
export type { QualityVerdict } from "@/lib/recommendation-quality";

// "For You" recommendation engine. Seeds are drawn from a user's own watched
// PlayHistory + WatchlistItem + MediaRequest, fanned out through the SERVER-WIDE
// recommendation graph (recommendation-graph.ts), scored, and cached per-user in
...
// than lossy: warmRecommendationsCache selects every user's seeds FIRST, hands
// that exact union to refreshRecommendationGraph as a REQUIRED set built without
// a per-run cap, and only then computes. A seed cannot be uncovered at compute
// time because the thing that decides what a seed is has already run.
//
// What that deliberately does NOT paper over: a source whose TMDB fetch failed
// during the build stays uncovered, and its seed contributes nothing this cycle.
// A fallback would have re-tried the same failing endpoint from the same process
...
// a sliver of a real library.
//
// What raising this costs, now that the graph is the only supplier: every seed
// is part of the REQUIRED set, which refreshRecommendationGraph builds without a
// per-run cap (guardrail 40). So this does not grow the graph — every watched
// title was already a source via collectGraphSources — it moves titles from the
...
// rating sources say. The prior itself — the per-source evidence weights, the
// confidence scaling and the obscurity damp — lives in recommendation-quality.ts,
// because a title's verdict is a property of the TITLE and the graph precomputes
// it server-wide (see applyQualityPrior below).
//
// Only the shortlist is rated: rating every candidate would mean thousands of
// lookups, and anything below the shortlist was never going to be stored
// anyway. Relevance gates, quality refines. Keep this equal to
// MAX_STORED_RECOMMENDATIONS_PER_USER — see the note there.
...
// How complete a seed set has to be before its answer is allowed to REPLACE the
...
// dead seed among a hundred is absorbed, while a real outage — which takes
// coverage down by far more than a fifth — holds the line.
const SEED_COVERAGE_FLOOR = 0.8;

function isConclusive(seedCount: number, coveredSeeds: number, rawSuggestions: number): boolean {
  if (coveredSeeds === seedCount) return true;
  return rawSuggestions > 0 && coveredSeeds >= seedCount * SEED_COVERAGE_FLOOR;
}
...
export interface RecommendationComputation {
  ...
  // A caller that REPLACES stored state must not treat an inconclusive empty as
  // authoritative, or one bad cron run wipes every user's recommendations and
  // reports success.
  //
  // Derived from SEED COVERAGE (see isConclusive), not from whether anything came
  // back. Every seed answered ⇒ authoritative, even if the answer is empty: the
  // graph distinguishes "TMDB has nothing similar" (a covered source with no
  // edges) from "not built" (no node stamp), so an empty result is a real result.
  // Below full coverage the answer is only trusted if the overwhelming majority
  // of seeds answered AND something came of them.
  //
  // Cheap to be wrong about in the safe direction: the caller just keeps
  // yesterday's recommendations for one more cycle.
  conclusive: boolean;
}

// One user's seed selection, resolved once and reused.
//
// warmRecommendationsCache resolves these for every user BEFORE the graph
// refresh, hands their union to the refresh as the required source set, and then
// passes each one back in here. That is the whole no-fallback guarantee in one
// sentence: the set of titles the graph must cover is not predicted, it is the
// literal set this pass will read, decided before the build ran.
//
// Re-selecting here instead would reopen a small hole — a watchlist add landing
// between the build and the compute would introduce a seed nothing had built.
export interface UserSeedPlan {
  linkedServerUserIds: string[];
  seeds: Seed[];
}

export async function selectSeedPlan(userId: string): Promise<UserSeedPlan> {
  const linkedServerUserIds = await resolveLinkedMediaServerUserIds(userId);
  return { linkedServerUserIds, seeds: await selectSeeds(userId, linkedServerUserIds) };
}

// Pure(ish) compute — no writes, and no upstream call of any kind. Returns no
// candidates for a cold-start user (zero eligible seeds). Read `conclusive`
// before acting on an empty `candidates`: the two empties mean different things.
//
// Pass `plan` whenever the caller has already selected the seeds AND built the
// graph for them (warmRecommendationsCache does both). Called without one, this
// selects its own seeds and can only report on whatever the graph happens to
// already hold — correct, but not guaranteed complete.
export async function computeRecommendationsForUser(
  userId: string,
  plan?: UserSeedPlan,
): Promise<RecommendationComputation> {
  const { linkedServerUserIds, seeds } = plan ?? (await selectSeedPlan(userId));
  if (seeds.length === 0) {
  ...
  // into "keep yesterday's shelf".
  const graphed = await readGraphSuggestions(
    seeds.map((s) => ({ tmdbId: s.tmdbId, mediaType: s.mediaType })),
  ).catch((err) => {
    console.error("[recommendations] graph read failed; this user keeps their existing shelf:", err);
    return new Map<string, TmdbMedia[]>();
  });

  // How many of this user's seeds the graph could actually answer for. The whole
  // trustworthiness of the result rests on this ratio (see `conclusive`), and it
  // is only knowable because coverage is a stored fact rather than the outcome of
  // a fetch we just attempted.
  let coveredSeeds = 0;
  ...
  seeds.forEach((seed) => {
    const seedKey = candidateKey(seed.tmdbId, seed.mediaType);
    const suggestions = graphed.get(seedKey);
    // undefined, not empty: the graph has no answer for this seed. An empty list
    // IS an answer ("TMDB knows of nothing similar") and counts as covered — the
    // distinction is the node stamp, and collapsing the two would make a title
    // with genuinely no suggestions look like an outage forever.
  ...
  return { candidates: ranked, conclusive: isConclusive(seeds.length, coveredSeeds, rawSuggestions) };
}
...
// candidate row here, so it is re-applied per candidate rather than baked in
// (see the voteAverage: 0 note in recommendation-graph.ts).
async function applyQualityPrior(shortlist: (RecommendationCandidate & { language?: string; voteCount: number })[]): Promise<void> {
  if (shortlist.length === 0) return;

  let precomputed: Awaited<ReturnType<typeof readTitleQuality>>;
  try {
    precomputed = await readTitleQuality(
      shortlist.map((c) => ({ tmdbId: c.tmdbId, mediaType: c.mediaType })),
    );
  } catch (err) {
    console.warn("[recommendations] quality read failed; ranking on relevance alone:", err);
    return;
  }

  for (const candidate of shortlist) {
    const row = precomputed.get(candidateKey(candidate.tmdbId, candidate.mediaType));
    // No verdict — the graph has not rated this title yet. That is an ABSENCE OF
    // INFORMATION, not a bad review: leave the relevance rank alone. The
    // obscurity damp fires only on a title that WAS asked about and got nothing,
    // which is why the graph stores those two states separately.
    if (!row) continue;
    const verdict = row.quality === null ? null : { quality: row.quality, evidence: row.evidence };
    // TMDB's own score is re-blended per candidate: the stored verdict is
    // TMDB-free, and this candidate's voteAverage/voteCount are right here.
    candidate.score *= qualityMultiplier(
      withTmdbTerm(verdict, candidate.voteAverage, candidate.voteCount),
      candidate.voteCount,
    );
  }
}
...
export interface WarmRecommendationsResult {
  usersEligible: number;
  usersUpdated: number;
  usersSkipped: number;
  usersFailed: number;
  candidatesWritten: number;
  // True when the graph refresh threw outright. The fan-out is skipped in that
  // case — see below.
  graphFailed: boolean;
  graph: GraphRefreshResult;
}

const NO_GRAPH: GraphRefreshResult = {
  sources: 0,
  requiredSources: 0,
  requiredCovered: 0,
  sourcesRefreshed: 0,
...
export async function warmRecommendationsCache(): Promise<WarmRecommendationsResult> {
  ...
  // Seeds are selected for the whole cohort BEFORE anything is built. Their
  // union is the exact set of titles the compute phase will ask the graph
  // about, which is what lets the build be a guarantee rather than a bet.
  const planResults = await settleLimit(userIds, USER_CONCURRENCY, async (userId) => ({
    userId,
    plan: await selectSeedPlan(userId),
  }));

  const plans: { userId: string; plan: UserSeedPlan }[] = [];
  let usersFailed = 0;
  for (const r of planResults) {
    if (r.status === "fulfilled") plans.push(r.value);
    else {
      usersFailed++;
      console.error("[recommendations] seed selection failed:", r.reason);
    }
  }

  const required = new Map<string, { tmdbId: number; mediaType: MediaType }>();
  for (const { plan } of plans) {
    for (const seed of plan.seeds) {
      required.set(candidateKey(seed.tmdbId, seed.mediaType), { tmdbId: seed.tmdbId, mediaType: seed.mediaType });
    }
  }

  // ── Phase 2: build ───────────────────────────────────────────────────────
  // A throw here ABORTS the run instead of computing anyway. Without a live
  // fallback an unbuilt graph produces zero coverage for everyone, which
  // `conclusive` correctly reads as "keep yesterday's shelf" — so the fan-out
  // would be several thousand queries whose only possible outcome is to change
  ...
  const results = await settleLimit(plans, USER_CONCURRENCY, async ({ userId, plan }) => {
    ...
    // graph would clear the user's shelf — and because the compute RESOLVES rather
    // than throws, it would be counted as a successful update. Keep the stale set
