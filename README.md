...
- Issues: the reporter now gets a push notification when their issue is resolved.
- Native (iOS) media detail: the "More like this" and collection rails now carry availability marks, and the response reports whether the viewer already holds a 4K request — powering the app's availability dots and full Request-in-4K flow.

**Fixed**

- Requests: editing an admin reply no longer requires a status change (the web "Edit reply" button returned a validation error).
- Native (iOS) media detail: 4K state is now scoped to the title's media type, so a TV page no longer offers 4K requests when only a Radarr 4K instance is configured (and vice-versa).
...
- Web: self-service **Delete Account** from your profile — anonymizes your account and signs you out (parity with the iOS app).
- Groundwork for **iOS push notifications** — APNs device registration (`/api/push/apns`), relay-based delivery, and end-to-end-encrypted notification payloads so notification text never passes through the relay in the clear. Inactive until the Summonarr iOS app ships.

### v0.13.4

**Added**

- **Account self-deletion** — permanently delete your own account and personal data from the iOS app (Profile → Delete Account). Your requests, votes, and issues are retained but de-identified, so the server's history stays intact.
- **Report content** — flag an issue message for the server admin to review from the iOS app; reports appear in the admin Audit Log.

**Fixed**

- Docker: rebuilt the Alpine base layer so the image ships patched libssl3/libcrypto3 (3.5.7-r0).
