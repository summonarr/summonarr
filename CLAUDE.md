...
- Grades each user A–F (A 80%+, B 60%+, C 40%+, D 20%+) on the share of their AVAILABLE requests they went on to watch, from `PlayHistory`. **Display-only** by product decision: a chip on Admin → Users and beside each requester in the request queue, opening the per-request breakdown from `GET /api/admin/users/[id]/watch-grade` (MANAGE_USERS or MANAGE_REQUESTS). Nothing reads it to gate requests, quotas or auto-approve.
- Computed on demand in one batch (no table, no cron). Tuned by `watchGradeGraceDays` / `watchGradeWindowDays` / `watchGradeTvPercent` (Settings → Media → Watch Grades) and switched off by `feature.behavior.watchGrades`; hidden entirely while play history tracking is off. Guardrail 34a has the rules.
...
34a. **The request watch grade scores only MATURED, COVERED requests, reads attribution through `linkedIdentityBranches`, and never gives an F to a user whose watches can't be seen.** Rules in [watch-grade.ts](src/lib/watch-grade.ts), data in [watch-grade-data.ts](src/lib/watch-grade-data.ts).
    ...
    - Both test files were mutation-verified: counting grace watches, dropping the coverage or since-request bound, treating unlinked users as observable, ignoring subjects, a float `ceil`, and averaging rounded credits each fail a named test.
