...
35. **A Plex/Jellyfin server is identified by a `serverInstance` slug string, and availability is a UNION across every configured server — NOTHING routes to a specific one.** Mirrors guardrail 32's arr model with that one deliberate divergence: an arr *request* must land on exactly one instance, whereas the only question multi-server answers here is "is this watchable somewhere, and what's playing right now."
    ...
    - **De-registering an instance must clean up its derived rows** — `/api/admin/media-instances`'s removal path deletes that slug's `PlexLibraryItem`/`JellyfinLibraryItem` and `ActiveSession` rows and its full Setting-key set, inside the same transaction as the registry write. Without it those library rows are unreachable by every writer (no sync path targets a de-registered slug) and fully visible to every reader (availability is an unscoped union), so **the removed server's entire catalogue reads "in library" forever**. `MediaServerUser` is soft-deleted and `PlayHistory` untouched (guardrails 28/19). The cleanup is not retroactive: a deployment that removed an instance before this shipped still has orphans.
    ...
    - **Still default-only by design (not gaps):** the per-source `/api/sync/plex` + `/api/sync/jellyfin` Resync routes (named instances sync via the orchestrator only — their deletes are pinned to `""` so they can't clobber a named instance), Plex reachability (`plexServerReachable` + the `plex:reachability` SSE event + the admin badge are single-server plumbing), the `plexLibraries`/`jellyfinLibraries` selection Setting and the strip-prefix editor (one admin-facing setting shared across instances; bad-matches reads a per-instance key with fallback to it), `/api/sync/tv-episodes`, library-scan's post-approve trigger, and the webhook presence probes. **Per-user server-visibility grants are deliberately NOT built**: availability is computed once and consumed by both a user-scoped read path and the userless notification path, whose exactly-once CAS (guardrail 14) and single `stillPending` snapshot (guardrail 15) are structurally incompatible with a per-user answer — that trade-off must be decided before any code, so don't add a half-wired `mediaServerGrants` field.
