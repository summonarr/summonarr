...
34a. **The request watch grade scores only MATURED, COVERED requests, reads attribution through `linkedIdentityBranches`, and never gives an F to a user whose watches can't be seen.** Rules in [watch-grade.ts](src/lib/watch-grade.ts), data in [watch-grade-data.ts](src/lib/watch-grade-data.ts).
    ...
    - Only plays at or after the request's `createdAt` count. TV credit is measured against REGULAR-season library episodes (specials excluded, integer ceil), and summed plays count toward "watched" so chunked viewing isn't marked unwatched.
    ...
    - **Other viewers are PEOPLE, counted through the same linkage rule.** A request the requester didn't watch counts as watched once `watchGradeOtherViewers` other people watched it since the request, each to the requester's own full-credit bar (`creditFor` over their plays). The audience aggregate returns media-server identities; the requester's own `linked` identities are dropped, and every other identity is keyed by the account that owns it via `resolveMediaServerUserOwners` ([my-watch-history.ts](src/lib/my-watch-history.ts)). So one account's Plex and Jellyfin logins count once, and a login an admin pinned away from the requester counts as someone else. When an FK link and an unpinned subject match claim one login, the FK account owns it.
    - Other viewers only ever RAISE a request's credit. They never score a grace or untracked request, and `0` turns the rule off — every viewer count is ≥ 0, so the `> 0` guard in `gradeUser` is load-bearing.
