...
32. **A Radarr/Sonarr instance is identified by an `arrInstance` slug string, NEVER the old `is4k` boolean.** The two-instance (HD/4K) model was generalized to N named instances (e.g. an "anime" instance).
    ...
    - **Still HD/4K-only (documented follow-ups, backend is N-instance-ready):** the trash-guides admin UI + the admin approve quality-profile picker (`?is4k=`), the admin request-list badge, bulk "Request all" + Discord routing (default instance only), and the per-instance grants editor + request-override selector UIs.
