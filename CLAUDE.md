...
- Grades each user A–F (A 80%+, B 60%+, C 40%+, D 20%+) on the share of their AVAILABLE requests they went on to watch, from `PlayHistory`. A request they skipped still counts as watched once enough OTHER people watched it (`watchGradeOtherViewers`, default 2). **Display-only** by product decision: a chip on Admin → Users and beside each requester in the request queue, opening the per-request breakdown from `GET /api/admin/users/[id]/watch-grade` (MANAGE_USERS or MANAGE_REQUESTS). Nothing reads it to gate requests, quotas or auto-approve.
- Computed on demand in one batch (no table, no cron). Tuned by `watchGradeGraceDays` / `watchGradeWindowDays` / `watchGradeTvPercent` / `watchGradeOtherViewers` (Settings → Media → Watch Grades) and switched off by `feature.behavior.watchGrades`; hidden entirely while play history tracking is off. Guardrail 34a has the rules.
