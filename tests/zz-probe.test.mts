...
test("PROBE: named-instance full resync — what does createMany actually write?", async () => {
  configurePlex();
  settings.set("plexRemoteServerUrl", PLEX_BASE);
  settings.set("plexRemoteAdminToken", "plex-admin-token-remote");
  settings.set("plexInstances", JSON.stringify([{ slug: "remote", name: "Remote" }]));
  ...
  const creates = opsFor("plexLibraryItem", "createMany");
  const deletes = opsFor("plexLibraryItem", "deleteMany");
  process.stdout.write("PROBE_DELETES " + JSON.stringify(deletes.map((d) => d.args)) + "\n");
  process.stdout.write("PROBE_CREATES " + JSON.stringify(creates.map((c) => c.args)) + "\n");
  const rows = creates.flatMap((c) => (c.args as { data: Record<string, unknown>[] }).data);
  process.stdout.write("PROBE_HAS_SERVERINSTANCE " + JSON.stringify(rows.map((r) => ("serverInstance" in r ? r.serverInstance : "<ABSENT>"))) + "\n");
});
