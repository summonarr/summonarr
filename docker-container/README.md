...
| `SUMMONARR_VERSION`            | image tag; default `latest`      | Pin the GHCR image tag. Example: `SUMMONARR_VERSION=0.23.1` (no `v` — the published tags are bare semver).                                                                                                   |
...
- You're on a Summonarr image **≥ `0.11.1`** (the script was added in that release). Pin with `SUMMONARR_VERSION=0.11.1` in `.env`, or run `:latest`.
